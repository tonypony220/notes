#!/bin/bash
#source https://github.com/f0rkr/k8s-42-session-setup/blob/main/setup.sh
...
#if [ ! -d $BREW_GOINFRE ]
#then
#	echo " $(tput setaf 1)------- setup HomeBrew -------$(tput sgr 0) "
...
#	git clone --depth=1 https://github.com/Homebrew/brew $BREW_GOINFRE && export PATH=$HOME/goinfre/.brew/bin:$PATH && brew update

#fi
#[ ! -d $DOCKER_GOINFRE ] && echo " $(tput setaf 1)------- Setup docker --------$(tput sgr 0) " && brew install docker docker-machine
...
#[ ! -d $MINIKUBE_GOINFRE ] && echo " $(tput setaf 1)------- Setup k8s + minikube ---------$(tput sgr 0)" && brew install minikube 
